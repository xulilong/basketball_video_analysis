...
面向篮球爱好者的独立原生安卓 App。包名 `com.courtmoments.app`，最低 Android 8.0，当前内测版 0.2.2。
...
- 选择或拍摄视频后先本地预览：显示文件名、大小、时长，支持播放/暂停、拖动进度、重新选择和取消；只有点击“确认上传”才提交视频。
- 已上传但未创建记录的视频，可在同一设备继续；视频分片上传后提交分析与剪辑；历史记录可恢复查看处理进度、个人/队伍得分。
- 上传完成的模式选择页、待分析球局和分析结果页均可播放原视频。
- 检测球员以截图卡片展示，支持内嵌图片和受保护图片链接；显示出现时间及关联状态，可跳转原视频对应时间核对，截图缺失或加载失败有明确提示。
