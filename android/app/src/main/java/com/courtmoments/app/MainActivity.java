...
public class MainActivity extends Activity {
    ...
    private void modeScreen(){
        add(body,heading("这次，记录谁的精彩？"));if(!pendingVideo.isEmpty())videoCard(pendingVideo,pendingName);
    ...
    private void loadImage(ImageView img,String path){if(!path.startsWith("/api/")&&!path.startsWith("data:image/"))return;final Api current=api;images.execute(()->{try{byte[] b=imageBytes(current,path);Bitmap bitmap=BitmapFactory.decodeByteArray(b,0,b.length);handler.post(()->{if(!isDestroyed()&&api==current)img.setImageBitmap(bitmap);});}catch(Exception ignored){}});}
    ...
    private void renderRecord(boolean preserve){
        ...
        String previewId=video==null?r.optString("draftVideoId",""):video.optString("id");if(!previewId.isEmpty())videoCard(previewId,video==null?"本场上传的视频":video.optString("name","本场上传的视频"));
    ...
    private void videoCard(String id,String name){LinearLayout c=card();add(c,heading("已上传的视频"));add(c,text(name,14,MUTED));add(c,button("▶ 播放原视频",false,()->play("/api/videos/"+id+"/media")));add(c,text("可随时回看原视频，暂停或拖动进度检查画面。",12,MUTED));}
    private void play(String path){playAt(path,0);}
    private void playAt(String path,int position){VideoView video=new VideoView(this);MediaController controls=new MediaController(this);controls.setAnchorView(video);video.setMediaController(controls);Map<String,String> headers=new HashMap<>();headers.put("Cookie",api.cookie());video.setVideoURI(Uri.parse(api.origin+path),headers);AlertDialog dialog=new AlertDialog.Builder(this).setView(video).setPositiveButton("关闭",(d,w)->video.stopPlayback()).create();dialog.setOnDismissListener(d->video.stopPlayback());video.setOnErrorListener((mp,a,b)->{message("暂时无法播放，可下载后查看");return true;});video.setOnPreparedListener(mp->{if(position>0)video.seekTo(position);video.start();});dialog.show();dialog.getWindow().setLayout(-1,dp(360));}
    ...
    private void identityDialog(){
        JSONObject video=detail.optJSONObject("video");if(video==null)return;String id=video.optString("id");
        run("正在读取检测球员",()->api.get("/api/workbench"),work->{
            JSONArray videos=work.getJSONArray("videos");JSONObject target=null;
            for(int i=0;i<videos.length();i++)if(videos.getJSONObject(i).optString("id").equals(id))target=videos.getJSONObject(i);
            if(target==null||target.optJSONObject("result")==null){message("请等待分析完成");return;}
            JSONArray detected=target.getJSONObject("result").optJSONArray("players");
            if(detected==null||detected.length()==0){message("本视频暂无检测球员，可先回看原视频。");return;}
            LinearLayout list=new LinearLayout(this);list.setOrientation(LinearLayout.VERTICAL);list.setPadding(dp(16),dp(8),dp(16),dp(8));
            add(list,text("先对照截图找到画面中的人，再选择本场球员。",14,MUTED));
            ScrollView sc=new ScrollView(this);sc.addView(list);
            AlertDialog dialog=new AlertDialog.Builder(this).setTitle("选择需要关联的检测球员").setView(sc).setNegativeButton("关闭",null).create();
            JSONObject associations=target.optJSONObject("associations");
            for(int i=0;i<detected.length();i++){
                JSONObject person=detected.getJSONObject(i);LinearLayout row=new LinearLayout(this);row.setOrientation(LinearLayout.VERTICAL);row.setPadding(dp(12),dp(12),dp(12),dp(8));row.setBackground(shape(BG,14));add(list,row);
                detectedPhoto(row,person,180);
                add(row,text("检测球员 "+(i+1)+" · "+Math.round(person.optDouble("first"))+" 秒出现",16,INK));
                String linked=associations==null?"":associations.optString(person.optString("id"));String name="尚未关联";JSONArray members=detail.optJSONArray("members");
                for(int j=0;j<members.length();j++)if(linked.equals(members.optJSONObject(j).optString("personId")))name="已关联："+members.optJSONObject(j).optString("name");
                add(row,text(name,13,MUTED));
                add(row,button("查看出现画面",false,()->playAt("/api/videos/"+id+"/media",(int)(person.optDouble("first")*1000))));
                add(row,button("关联这位球员",true,()->{dialog.dismiss();linkIdentity(id,person);}));
            }
            dialog.show();dialog.getWindow().setLayout(-1,(int)(getResources().getDisplayMetrics().heightPixels*0.85));
        });
    }
    private byte[] imageBytes(Api client,String path) throws Exception {
        if(path.startsWith("data:image/")){int split=path.indexOf(',');if(split<0||!path.substring(0,split).endsWith(";base64")||path.length()>12000000)throw new IOException("无效截图");return android.util.Base64.decode(path.substring(split+1),android.util.Base64.DEFAULT);}
        return client.request("GET",path,null,null,null);
    }
    private void detectedPhoto(LinearLayout parent,JSONObject person,int height){
        ImageView img=new ImageView(this);img.setScaleType(ImageView.ScaleType.FIT_CENTER);img.setBackground(shape(0xffe6efec,12));img.setContentDescription("检测球员截图");parent.addView(img,new LinearLayout.LayoutParams(-1,dp(height)));
        TextView state=text("正在加载截图…",12,MUTED);add(parent,state);String path=person.optString("photo");
        if(!path.startsWith("/api/")&&!path.startsWith("data:image/")){state.setText("暂无截图，请查看出现画面后确认");return;}
        final Api current=api;images.execute(()->{try{byte[] bytes=imageBytes(current,path);Bitmap bitmap=BitmapFactory.decodeByteArray(bytes,0,bytes.length);if(bitmap==null)throw new IOException();handler.post(()->{if(!isDestroyed()&&api==current){img.setImageBitmap(bitmap);state.setVisibility(View.GONE);}});}catch(Exception e){handler.post(()->{state.setText("截图加载失败，点击重试或查看出现画面");state.setOnClickListener(v->{parent.removeView(img);parent.removeView(state);detectedPhoto(parent,person,height);});});}});
    }
    private void linkIdentity(String video,JSONObject detected){
        LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(16),dp(8),dp(16),dp(8));detectedPhoto(box,detected,180);
        add(box,text("将这张截图中的人关联到：",15,INK));ScrollView sc=new ScrollView(this);sc.addView(box);
        AlertDialog dialog=new AlertDialog.Builder(this).setTitle("这位球员是谁？").setView(sc).setNegativeButton("返回",(d,w)->identityDialog()).create();
