...
管理员可从侧栏“任务队列”进入 `/admin/queue`，查看所有账号的视频分析、进球剪辑及选中片段导出状态。每 3 秒刷新，可手动刷新；显示当前处理/等待数量和最多 30 条已结束记录。服务端限制管理员访问，读取队列不会重新分析或改写视频数据。进程已退出的任务按异常展示；等待列表不承诺严格 FIFO 顺序，也不推算剩余时间。

任务队列支持管理员取消等待中或正在处理的任务。取消前显示账号、视频与任务类型确认，服务端校验当前任务进程与列表版本，停止对应工作进程组，保留原视频及此前完成的结果。已结束的任务不再提供取消按钮。取消后可从对应工具重新开始；操作不会取消其他任务。
